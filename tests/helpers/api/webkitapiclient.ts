...
export class WebkitApiClient implements IApiClient {
  ...
  setRequest(request: APIRequestContext) {
    ...
    if (global) {
      try {
        ...
        console.log('Stored WebKit request context in global.__lastWebkitRequest');
      } catch (e) {
  ...
  private getRequestContext(): APIRequestContext | null {
    ...
    try {
      ...
      if (global && global.__lastWebkitRequest) {
        console.log('Retrieved WebKit request context from global.__lastWebkitRequest');
        // @ts-ignore - We're accessing a global property
      ...
      if (global && global.__webkitRequestContext) {
        console.log('Retrieved WebKit request context from global.__webkitRequestContext');
        // @ts-ignore - We're accessing a global property
      ...
      for (const url of urlFormats) {
        ...
        while (retries > 0) {
          try {
            ...
            if (!requestContext) {
              console.error('Request context lost during retry, attempting to recover...');
            ...
            lastError = e;
            console.error(`Error during API call attempt to ${url} (${retries} retries left):`, e);
            
            // Special handling for closed browser errors
            if (e.toString().includes('Target page, context or browser has been closed')) {
              console.log('Detected closed browser error, attempting to recover request context...');
              
              // Try to refresh the request context if possible
              try {
                // @ts-ignore - Accessing global property
                if (global && global.__webkitRequestContext) {
                  console.log('Recovered request context from global.__webkitRequestContext');
                  this.request = global.__webkitRequestContext;
                }
              } catch (recoveryError) {
                console.warn('Failed to recover request context:', recoveryError);
              }
            }
          }
      ...
      for (const url of urlFormats) {
        ...
        while (retries > 0) {
          try {
            ...
            console.error(`Error getting metadata from ${url} (${retries} retries left):`, e);
      ...
      for (const url of urlFormats) {
        ...
        while (retries > 0) {
          try {
            ...
            // Special case: For 404, we don't need to retry
            if (response.status() === 404) {
              console.log(`Secret ${secretId} not found or access limit reached (404)`);
              return {
            ...
            console.error(`Error accessing secret from ${url} (${retries} retries left):`, e);
  ...
  async deleteSecret(secretId: string): Promise<null | IApiError> {
    ...
    try {
      ...
      // Try each URL with retries
      for (const url of urlFormats) {
        if (response && (response.ok() || response.status() === 204 || response.status() === 404)) break;
        ...
        while (retries > 0) {
          try {
            ...
            if (response.ok() || response.status() === 204 || response.status() === 404) {
              console.log(`Deleted secret successfully from ${url} (status ${response.status()})`);
              break;
            }
            ...
            console.error(`Error deleting secret from ${url} (${retries} retries left):`, e);
        ...
        // If we got a successful response, break out of the URL loop
        if (response && (response.ok() || response.status() === 204 || response.status() === 404)) {
          break;
        }
      }
      ...
      if (!response) {
        console.error('Failed to delete secret - no response after all retries');
        return {
      ...
      if (!response.ok()) {
        console.error(`Unexpected response status ${response.status()} when deleting secret ${secretId}`);
