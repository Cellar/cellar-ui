...
export function formatDate(date: Date | string): string {
  ...
  return date.toLocaleDateString(navigator.language, {month: '2-digit', day: '2-digit', year: 'numeric'})
}
...
export function getTimeZone(date: Date | string): string {
  if (typeof date === 'string')
    date = new Date(date)
  const parts = new Intl.DateTimeFormat(navigator.language, { timeZoneName: 'short' }).formatToParts(date);
