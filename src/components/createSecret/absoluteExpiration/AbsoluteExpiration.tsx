...
export const AbsoluteExpiration: FC<{
  ...
  function getAbsoluteParts(target: Date): {
    date: Date;
    time: string;
    amPm: string;
  } {
    ...
    if (minutes >= 0 && minutes < 15) {
      ...
      // minutes should round up the hour
      minutes = 0;
      hours++;
    }
  ...
  return (
    <>
      ...
      <FlatSelect
        ...
        onChange={(e) => {
          setExpiration(
            getAbsoluteDate(formatDateForParsing(date), e.target.value, amPm),
          );
        }}
      />
      <FlatSelect
        ...
        onChange={(e) => {
          setExpiration(
            getAbsoluteDate(formatDateForParsing(date), time, e.target.value),
          );
        }}
      />
    </>
